...
class HousingWebSocketServer implements MessageComponentInterface {
    ...
    public function onMessage(ConnectionInterface $from, $msg) {
        ...
        switch ($data['type']) {
            ...
            // NEW CRUD OPERATIONS FOR PROPERTIES
            case 'CREATE_PROPERTY':
                $this->handleCreateProperty($from, $data['payload']);
                break;
                
            case 'UPDATE_PROPERTY':
                $this->handleUpdateProperty($from, $data['payload']);
                break;
                
            case 'DELETE_PROPERTY':
                $this->handleDeleteProperty($from, $data['payload']);
                break;
                
            case 'ARCHIVE_PROPERTY':
                $this->handleArchiveProperty($from, $data['payload']);
                break;
    ...
    // ---------- NEW PROPERTY CRUD METHODS ----------
    
    private function handleCreateProperty($from, $payload) {
        if (!$this->pdo) {
            $from->send(json_encode([
                'type' => 'ERROR',
        ...
        if ($from->role !== 'seller') {
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Only sellers can create properties']
            ]));
            return;
        }
        ...
        try {
            ...
            // Optional fields
            $image_path = isset($payload['image_path']) ? $payload['image_path'] : null;
            
            // Insert new property
            $stmt = $this->pdo->prepare(
                "INSERT INTO properties (seller_id, title, description, price, location, image_path, is_archived) 
                 VALUES (:seller_id, :title, :description, :price, :location, :image_path, 0)"
            );
            ...
            // Fetch the newly created property to send back
            ...
            $fetchStmt->execute([':id' => $propertyId]);
            ...
            $this->log("Property created: ID {$propertyId} by user {$from->user_id}", "success");
            
            // Optionally broadcast to all connected clients that a new property is available
            $this->broadcastPropertyUpdate('PROPERTY_CREATED', $property);
            
        } catch (\Exception $e) {
            $this->errorCount++;
            $this->log("Error creating property: " . $e->getMessage(), "error");
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Failed to create property: ' . $e->getMessage()]
            ]));
        }
    }
    
    private function handleUpdateProperty($from, $payload) {
        if (!$this->pdo) {
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Database connection unavailable']
        ...
        if (!isset($payload['property_id'])) {
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Missing property_id']
            ]));
            return;
        }
        
        try {
            // Check if property exists and belongs to this seller
            $checkStmt = $this->pdo->prepare(
                "SELECT * FROM properties WHERE id = :id"
            );
            $checkStmt->execute([':id' => $payload['property_id']]);
            ...
            // Authorization: only the owner (seller) can update
            if ($property['seller_id'] != $from->user_id) {
                $from->send(json_encode([
                    'type' => 'ERROR',
                    'payload' => ['message' => 'Unauthorized: you do not own this property']
                ]));
                return;
            }
            ...
            $updateFields = [];
            $params = [':id' => $payload['property_id']];
            
            $allowedFields = ['title', 'description', 'price', 'location', 'image_path'];
            ...
            $sql = "UPDATE properties SET " . implode(', ', $updateFields) . " WHERE id = :id";
            ...
            $fetchStmt->execute([':id' => $payload['property_id']]);
            $updatedProperty = $fetchStmt->fetch(\PDO::FETCH_ASSOC);
            ...
            $from->send(json_encode($response));
            $this->log("Property updated: ID {$payload['property_id']} by user {$from->user_id}", "success");
            
            // Optionally broadcast update to all clients
            $this->broadcastPropertyUpdate('PROPERTY_UPDATED', $updatedProperty);
            
        } catch (\Exception $e) {
            $this->errorCount++;
            $this->log("Error updating property: " . $e->getMessage(), "error");
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Failed to update property: ' . $e->getMessage()]
            ]));
        }
    }
    
    private function handleDeleteProperty($from, $payload) {
        // Hard delete – use with caution, usually archive is better
        if (!$this->pdo) {
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Database connection unavailable']
        ...
        try {
            // Check ownership
            $checkStmt = $this->pdo->prepare(
                "SELECT seller_id FROM properties WHERE id = :id"
            );
            $checkStmt->execute([':id' => $payload['property_id']]);
            $property = $checkStmt->fetch(\PDO::FETCH_ASSOC);
            ...
            // Delete property
            $deleteStmt = $this->pdo->prepare("DELETE FROM properties WHERE id = :id");
            $deleteStmt->execute([':id' => $payload['property_id']]);
            
            $response = [
                'type' => 'PROPERTY_DELETED',
                'payload' => ['property_id' => $payload['property_id']]
            ];
            
            $from->send(json_encode($response));
            $this->log("Property deleted: ID {$payload['property_id']} by user {$from->user_id}", "success");
            
            // Notify all clients that property is gone
            $this->broadcastPropertyUpdate('PROPERTY_DELETED', ['property_id' => $payload['property_id']]);
            
        } catch (\Exception $e) {
            $this->errorCount++;
            $this->log("Error deleting property: " . $e->getMessage(), "error");
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Failed to delete property: ' . $e->getMessage()]
            ]));
        }
    }
    
    private function handleArchiveProperty($from, $payload) {
        ...
        try {
            ...
            $archiveStmt = $this->pdo->prepare(
                "UPDATE properties SET is_archived = 1 WHERE id = :id"
            );
            $archiveStmt->execute([':id' => $payload['property_id']]);
            
            $response = [
                'type' => 'PROPERTY_ARCHIVED',
                'payload' => ['property_id' => $payload['property_id'], 'is_archived' => 1]
            ];
            
            $from->send(json_encode($response));
            $this->log("Property archived: ID {$payload['property_id']} by user {$from->user_id}", "success");
            
            // Broadcast archive event
            $this->broadcastPropertyUpdate('PROPERTY_ARCHIVED', ['property_id' => $payload['property_id']]);
            
        } catch (\Exception $e) {
            $this->errorCount++;
            $this->log("Error archiving property: " . $e->getMessage(), "error");
            $from->send(json_encode([
                'type' => 'ERROR',
                'payload' => ['message' => 'Failed to archive property: ' . $e->getMessage()]
            ]));
        }
    }
    
    // Helper to broadcast property updates to all connected clients (optional)
    private function broadcastPropertyUpdate($type, $data) {
        $message = json_encode([
            'type' => $type,
            'payload' => $data
        ]);
        ...
        foreach ($this->clients as $client) {
            // Optionally, you can filter clients that are authenticated and have relevant role
            $client->send($message);
        }
    }
    
    // ---------- END NEW PROPERTY CRUD METHODS ----------
