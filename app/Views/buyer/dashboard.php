...
<body class="bg-light">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-success shadow-sm">
        <div class="container">
            <div class="d-flex align-items-center">
                ...
                <a href="<?= base_url('/profile/' . $user['id']) ?>" class="btn btn-outline-light btn-sm">👤 View
                    Profile</a>
            </div>
    ...
    <div class="container my-4">
        ...
        <h3 class="text-success fw-bold mb-3">
            ...
            <small class="text-muted fs-6 ms-2" id="autoRefreshBadge">(Auto-refresh every 5s)</small>
        </h3>
            ...
            <div class="col-md-4">
                <div class="input-group">
                    ...
                    <input type="text" name="search" id="search" class="form-control"
                        placeholder="Search by title or description" value="<?= esc($search ?? '') ?>">
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="input-group">
                    ...
                    <input type="text" name="location" id="location" class="form-control"
                        placeholder="Search by location" value="<?= esc($location ?? '') ?>">
                </div>
            </div>
    ...
    function addDebugLog(message, data = null) {
        ...
        if (debugDiv.length) {
            debugDiv.html(debugLogs.map(log =>
                    `<div style="border-bottom: 1px solid #333; padding: 2px 0;">${escapeHtml(log)}</div>`).join(
                ''));
        }
    ...
    // 5-second auto-refresh AJAX timer
    ...
    function handleWebSocketMessage(data) {
        switch (data.type) {
            ...
            case 'OFFER_UPDATED':
                ...
                showToast(`📝 Your offer has been ${statusText}`, data.payload.status === 'accepted' ? 'success' :
                    'warning');
                loadProperties();
                break;
    ...
    // Load properties via AJAX (with cache busting)
    let isLoadingProperties = false; // prevent overlapping calls
    function loadProperties() {
        // Prevent multiple simultaneous loads
        if (isLoadingProperties) {
            addDebugLog('loadProperties already in progress, skipping');
            return;
        }
        ...
        addDebugLog(
            `Auto-refresh: Loading properties - Search: "${search}", Location: "${location}", Price: "${price_range}"`
            );
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                // silent fail, do not spam toast on auto-refresh errors to avoid annoyance
                if (status !== 'abort') {
                    // only show on user-triggered? but we'll limit to occasional
                    if (autoRefreshInterval) {
                        // it's auto-refresh, less noisy
    ...
    function updatePropertyListings(properties) {
        ...
        if (!properties || properties.length === 0) {
            html =
                '<div class="col-12 text-center text-muted py-5"><i class="fas fa-home fa-3x mb-3"></i><p>No properties found.</p></div>';
        } else {
    ...
    $(document).ready(function() {
        ...
        // ***** ADD 5-SECOND AUTO-REFRESH AJAX TIMER *****
        ...
        autoRefreshInterval = setInterval(function() {
            addDebugLog('⏲️ 5-second auto-refresh trigger');
            loadProperties();
        }, 5000);
        addDebugLog('✅ Auto-refresh timer set: every 5 seconds');

        // Optional: Add visual indicator badge animation (just for fun)
    ...
    // Clean up intervals on page unload to avoid background requests
    window.addEventListener('beforeunload', function() {
        if (ws) ws.close();
        if (pollingInterval) clearInterval(pollingInterval);
        if (heartbeatInterval) clearInterval(heartbeatInterval);
        if (autoRefreshInterval) clearInterval(autoRefreshInterval);
        addDebugLog('Cleanup: cleared all intervals');
    });
    ...
    // Also expose manual refresh for debugging (optional)
