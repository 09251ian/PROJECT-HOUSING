...
class SellerController extends BaseController
{
    ...
    public function upload_image()
    {
        ...
        if (!$file || !$file->isValid()) {
            return $this->response->setJSON(['success' => false, 'error' => 'No valid image uploaded']);
        }

        // Use public directory (FCPATH) for images
        $uploadPath = FCPATH . 'uploads/properties/';
        if (!is_dir($uploadPath)) {
            if (!mkdir($uploadPath, 0777, true)) {
                return $this->response->setJSON(['success' => false, 'error' => 'Cannot create upload directory']);
            }
        }
        ...
        if ($file->move($uploadPath, $newName)) {
            ...
            return $this->response->setJSON(['success' => true, 'image_path' => $imagePath]);
        }
    ...
    public function offerAction()
    {
        ...
        if ($action === 'accept') {
            ...
            if ($this->request->isAJAX()) {
                return $this->response->setJSON(['success' => false, 'error' => 'Invalid action.']);
            }
    ...
    public function archive()
    {
            ...
            if (!$propertyId) {
                if ($this->request->isAJAX()) {
                    return $this->response->setJSON(['success' => false, 'error' => 'Invalid property id.']);
                }
            ...
            if (!$property) {
                if ($this->request->isAJAX()) {
                    return $this->response->setJSON(['success' => false, 'error' => 'Property not found.']);
                }
                session()->setFlashdata('error', 'Property not found.');
                return redirect()->to('/seller/dashboard');
            }
